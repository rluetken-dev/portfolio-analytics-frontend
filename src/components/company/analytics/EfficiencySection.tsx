...
const fmtPercent = (v: number | null | undefined) => (v == null || !Number.isFinite(v) ? "—" : `${(v * 100).toFixed(1)}%`);
const fmtRatio = (v: number | null | undefined) => (v == null || !Number.isFinite(v) ? "—" : `${v.toFixed(2)}x`);
...
export default function EfficiencySection({ symbol }: { symbol: string }) {
  ...
  React.useEffect(() => {
    let cancelled = false;
    async function run() {
      setAt("—"); setCagr("—"); setCount(`0/${TOTAL}`);
      if (!sym) return;
      try {
        const [atR, cgR] = await Promise.all([
          fetchMetricNumber(backendBase, "/api/analytics/asset-turnover", sym, ["value","assetTurnover"]),
          fetchMetricNumber(backendBase, "/api/analytics/equity-cagr", sym, ["value","equityCagr","cagr"]),
        ]);
        if (cancelled) return;
